import { NextRequest, NextResponse } from "next/server";
import { db } from "@/lib/db";
import { handleApiError } from "@/lib/api-error";
import { requireInternalAdminSession } from "@/lib/auth/internal-admin";
import { getRequestActorMeta } from "@/lib/internal-admin/document-review";
import { markDocumentReviewFailed } from "@/lib/internal-admin/document-review-service";
import {
  parseFailedReason,
  readOptionalJson,
} from "@/lib/internal-admin/document-review-validation";

// POST /api/internal-admin/documents/:documentId/mark-failed
// Body: { reason: string }
// Marks PROCESSING (or WAITING_VERIFIED_APPROVAL) document as FAILED with user-safe reason.
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ documentId: string }> },
) {
  const { documentId } = await params;
  try {
    const session = await requireInternalAdminSession();
    if (session instanceof NextResponse) return session;
    if (!db) return NextResponse.json({ error: "Service unavailable" }, { status: 503 });

    const body = await readOptionalJson<{ reason?: unknown }>(req, {});
    const parsedReason = parseFailedReason(body);
    if (!parsedReason.ok) {
      return NextResponse.json({ error: parsedReason.error }, { status: parsedReason.status });
    }
    const result = await markDocumentReviewFailed({
      db,
      documentId,
      reason: parsedReason.reason,
      actor: {
        userId: session.userId,
        requestMeta: getRequestActorMeta(req),
      },
    });
    return NextResponse.json(result.ok ? { ok: true, ...result.data } : result.body, {
      status: result.ok ? 200 : result.status,
    });
  } catch (err) {
    return handleApiError(err, `POST /api/internal-admin/documents/${documentId}/mark-failed`);
  }
}
